...
class StartPage:
    def __init__(self, parent_window):
        ...
        Button(self.window, text="绩点查询", font=("华文中宋", 15), command=lambda: StudentPage_1(self.window), width=15,
               height=1,
               fg='black', activebackground='black', activeforeground='white').pack()
...
class StudentPage:
    ...
    def login(self):
        ...
        if self.student_pass.get() == stu_pass:
            StudentManage(self.window, self.student_id.get())  # 进入学生信息查看界面
            #StudentView(self.window, self.student_id.get())
        else:
            messagebox.showinfo('警告！', '用户名或密码不正确！')
...
class StudentPage_1:
    ...
    def login(self):
        ...
        if self.student_pass.get() == stu_pass:
            StudentGPA(self.window, self.student_id.get())  # 进入学生信息查看界面
            #StudentView(self.window, self.student_id.get())
        else:
            messagebox.showinfo('警告！', '用户名或密码不正确！')
...
class AdminManage:
    def __init__(self, parent_window):
        ...
        # 表格的标题
        self.tree.column("学号", width=140, anchor='center')  # 表示列,不显示
        self.tree.column("姓名", width=140, anchor='center')
        self.tree.column("性别", width=140, anchor='center')
        self.tree.column("年龄", width=140, anchor='center')
        self.tree.column("院系", width=140, anchor='center')
        ...
        # 定义顶部区域
        # 定义左上方区域
        self.top_title = Label(self.frame_left_top, text="学生信息列表:", font=('华文中宋', 18))
        ...
        # 学号
        self.right_top_id_label = Label(self.frame_left_top, text="  学号：", font=('华文中宋', 15))
        self.right_top_id_entry = Entry(self.frame_left_top, textvariable=self.var_id, font=('华文中宋', 15))
        self.right_top_id_label.grid(row=1, column=0)  # 位置设置
        self.right_top_id_entry.grid(row=1, column=1)
        # 姓名
        self.right_top_name_label = Label(self.frame_left_top, text="  姓名：", font=('华文中宋', 15))
        self.right_top_name_entry = Entry(self.frame_left_top, textvariable=self.var_name, font=('华文中宋', 15))
        self.right_top_name_label.grid(row=2, column=0)  # 位置设置
        self.right_top_name_entry.grid(row=2, column=1)
        # 性别
        self.right_top_gender_label = Label(self.frame_left_top, text="  性别：", font=('华文中宋', 15))
        ...
        # 年龄
        self.right_top_gender_label = Label(self.frame_left_top, text="  年龄：", font=('华文中宋', 15))
        ...
        # 院系
        self.right_top_gender_label = Label(self.frame_left_top, text="  院系：", font=('华文中宋', 15))
        self.right_top_gender_entry = Entry(self.frame_left_top, textvariable=self.var_dept, font=('华文中宋', 15))
        self.right_top_gender_label.grid(row=5, column=0)  # 位置设置
        self.right_top_gender_entry.grid(row=5, column=1)

        # 定义右上方区域
        self.right_top_title = Label(self.frame_right_top, text="修改学生信息表：", font=('华文中宋', 18))

        self.tree.bind('<Button-1>', self.click)  # 左键获取位置
        self.right_top_button1 = ttk.Button(self.frame_right_top, text='新建学生信息', width=40, command=self.new_row)
        self.right_top_button2 = ttk.Button(self.frame_right_top, text='更新选中学生信息', width=40,
                                            command=self.updata_row)
        self.right_top_button3 = ttk.Button(self.frame_right_top, text='删除选中学生信息', width=40,
...
class StudentManage:
    def __init__(self, parent_window, student_id):
        ...
        # 表格的标题
        self.tree.column("课程号", width=140, anchor='center')  # 表示列,不显示
        self.tree.column("课程名", width=140, anchor='center')
        self.tree.column("学分", width=140, anchor='center')
        self.tree.column("成绩", width=140, anchor='center')
        self.tree.column("授课教师", width=140, anchor='center')
        ...
        sql = "SELECT * FROM s_c WHERE sno = '%s'"%(student_id)  # SQL 查询语句
        try:
            # 执行SQL语句
            cursor.execute(sql)
            # 获取所有记录列表
        ...
        # 定义顶部区域
        # 定义左上方区域
        self.top_title = Label(self.frame_left_top, text="学号："+str(student_id), font=('华文中宋', 18))
        self.top_title.grid(row=0, column=0, columnspan=2, sticky=NSEW, padx=50, pady=10)
        ...
        # 学号
        self.right_top_id_label = Label(self.frame_left_top, text="课程号：", font=('华文中宋', 15))
        self.right_top_id_entry = Entry(self.frame_left_top, textvariable=self.var_id, font=('华文中宋', 15))
        self.right_top_id_label.grid(row=1, column=0)  # 位置设置
        self.right_top_id_entry.grid(row=1, column=1)
        # 姓名
        self.right_top_name_label = Label(self.frame_left_top, text="课程名：", font=('华文中宋', 15))
        self.right_top_name_entry = Entry(self.frame_left_top, textvariable=self.var_name, font=('华文中宋', 15))
        self.right_top_name_label.grid(row=2, column=0)  # 位置设置
        self.right_top_name_entry.grid(row=2, column=1)
        # 性别
        self.right_top_gender_label = Label(self.frame_left_top, text="学分：", font=('华文中宋', 15))
        self.right_top_gender_entry = Entry(self.frame_left_top, textvariable=self.var_gender,
                                            font=('华文中宋', 15))
        self.right_top_gender_label.grid(row=3, column=0)  # 位置设置
        self.right_top_gender_entry.grid(row=3, column=1)
        # 年龄
        self.right_top_gender_label = Label(self.frame_left_top, text="成绩：", font=('华文中宋', 15))
        self.right_top_gender_entry = Entry(self.frame_left_top, textvariable=self.var_age,
                                            font=('华文中宋', 15))
        self.right_top_gender_label.grid(row=4, column=0)  # 位置设置
        self.right_top_gender_entry.grid(row=4, column=1)
        # 院系
        self.right_top_gender_label = Label(self.frame_left_top, text="授课老师：", font=('华文中宋', 15))
        self.right_top_gender_entry = Entry(self.frame_left_top, textvariable=self.var_dept, font=('华文中宋', 15))
        self.right_top_gender_label.grid(row=5, column=0)  # 位置设置
        self.right_top_gender_entry.grid(row=5, column=1)

        # 定义右上方区域
        self.right_top_title = Label(self.frame_right_top, text="课程操作：", font=('华文中宋', 18))

        self.tree.bind('<Button-1>', self.click)  # 左键获取位置
        self.right_top_button1 = ttk.Button(self.frame_right_top, text='选课', width=40, command=self.new_row)
       # self.right_top_button2 = ttk.Button(self.frame_right_top, text='更新选中学生信息', width=20,
        #                                    command=self.updata_row)
        self.right_top_button3 = ttk.Button(self.frame_right_top, text='撤课', width=40,
                                            command=self.del_row)
    ...
    def new_row(self):
        ...
        if str(self.var_id.get()) in self.id:
            ...
            if self.var_id.get() != '' and self.var_name.get() != '' and self.var_gender.get() != '' and self.var_dept.get() != '':
                ...
                db = pymysql.connect("localhost", "root", "admin", "student")
                cursor = db.cursor()  # 使用cursor()方法获取操作游标
                sql = "INSERT INTO course_n(sno, id, name, gender, age, dept) \
    				       VALUES ('%s', ''%s', '%s', '%s', '%s', '%s')" % \
                      (self.sno, self.var_id.get(), self.var_name.get(), self.var_gender.get(), self.var_age.get(),
                ...
                messagebox.showinfo('警告！', '请填写必要数据')

    def del_row(self):
        res = messagebox.askyesnocancel('警告！', '是否撤掉该门课程？')
        if res == True:
            print(self.row_info[0])  # 鼠标选中的学号
            print(self.tree.selection()[0])  # 行号
            print(self.tree.get_children())  # 所有行
...
class StudentGPA:
    def __init__(self, parent_window, student_id):
        ...
        db = pymysql.connect("localhost", "root", "admin", "student")
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "SELECT * FROM s_c_copy1 WHERE sno = '%s'" % (student_id)  # SQL 查询语句
        #print('11111111111')
        try:
            #执行sql语句
            cursor.execute(sql)
            ...
            for row in results:
                #print(row[3])
                gpa_sum += row[3] * float(row[6])
                credit_sum += float(row[3])
                #print('gpa = '+str(gpa_sum))
                #print('学分='+str(credit_sum))
        except:
            print('error!')
        db.close()
        avg_gpa = gpa_sum / credit_sum
        Label(self.window, text=' 学号：'+str(student_id)+'\n总绩点：'+str(round(avg_gpa,5)),anchor='w', font=('华文中宋', 18)).pack(pady=5)
        Button(self.window, text="返回首页", width=8, font=tkFont.Font(size=16), command=self.back).pack(pady=25)
